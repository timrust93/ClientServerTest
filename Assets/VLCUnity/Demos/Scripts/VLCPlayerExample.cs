...
public class VLCPlayerExample : MonoBehaviour
{
    ...
    public string
        path =
            "http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4"; //Can be a web path or a local path
    ...
    void Update()
    {
        ...
        if (_vlcTexture != null)
        {
            ...
            if (updated)
            {
                ...
                _image.rectTransform.sizeDelta = new Vector2(_vlcTexture.width, _vlcTexture.height);
                _image.sprite = Sprite.Create(_vlcTexture, new Rect(0, 0, _vlcTexture.width, _vlcTexture.height),
                    Vector2.zero);
            }
        }
    }
